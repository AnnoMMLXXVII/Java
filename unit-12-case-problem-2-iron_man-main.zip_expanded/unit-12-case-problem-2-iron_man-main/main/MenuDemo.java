...
public class MenuDemo {
	...
	public static Appetizer[] createAppetizers() {
		...
		try {
			...
			System.out.println(negativePriceException.getMessage());
		}
		return appetizers;
	}
	...
	public static Side[] createSides() {
		...
		try { // TODO Create and add 4 sides (you can increase the number if you want)
	...
	public static MenuItem askForMenuItemSelection(Scanner input, MenuItem[] menuItems) {
		...
		while (true) {
			try {
				if (response < 0 || response > menuItems.length-1) {
					input.nextLine();
					throw new NegativePriceException();
				}else {break;}
			} catch (NegativePriceException negativepriceexception) {
				System.out.println(negativepriceexception);
			}
		}
