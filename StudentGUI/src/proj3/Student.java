...
public class Student implements Comparable<Student> {
     ...
     * parameters pFirstName and pLastName to instance variables by calling mutators
     * -- Note that we only create the exam list here, it will be populated later
     * create an ArrayList<Integer> and pass it off to setExamList() -- Note that we
     * only create the homework list here, it will be populated later create an
     * ArrayList<Integer> and pass it off to setHomeworkList() end Student()
     */
     ...
     *
     * @param pScore PSEUDOCODE: method addExam(pScore : int) : void call
     ...
     *
     * @param pScore PSEUDOCODE: method addHomework(pScore : int) : void call
     *               add(pScore) on getHomeworkList() to add a new homework score to
     *               the list of homework scores end addHomework
     */
     ...
     *                 <p>
     *                 Provide the annotation that prevents accidental overloading
     *                 since we are overriding the String.compareTo() method.
     *                 <p>
     *                 PSEUDOCODE: method compareTo(pStudent : Student) : int
     *                 return: negative int if the last name of this Student is <
     *                 the last name of pStudent return: zero if the last name of
     *                 this Student is = the last name of pStudent return: positive
     *                 int if the last name of this Student is > the last name of
     *                 pStudent hint: the last names are Strings and String already
    ...
    public String toString() {
        String result = mLastName + " " + mFirstName;
        // for(int i =0, i< getExamList().size(); i++){
        // result += " " + getExamList.get(i);
        // }
        for (int eScore : getExamList()) {
            result += " " + eScore;
        }
        // for(int i =0, i< getHomeoworkList().size(); i++){
        // result += " " + getHomeworkList.get(i);
        for (int hScore : getHomeworkList()) {
            result += " " + hScore;
        }
        return result;
    }
