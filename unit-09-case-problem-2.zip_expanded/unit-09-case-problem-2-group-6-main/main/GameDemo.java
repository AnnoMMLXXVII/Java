 ...
 * 
 * Purpose (Class Description): This will allow a user to play a Matching Game on their own and see
 * how many rounds it takes them to find all the pairs.
 */
import java.util.Scanner;

public class GameDemo {
  /**
   * This welcomes the play to the Matching Game, has the user play the game, and
   * then thanks them for playing.
   * 
   * @param args Takes in arguments from the console. They aren't used in this
   *             method.
   */
  ...
  /**
   * This creates a new matching game by making a new Grid. User will continue to
   * play until all matches are revealed. It will end with letting the user know
   * how many rounds it took them to win.
  ...
  /**
   * This will handle playing one round of the matching game. This includes the
   * user revealing two valid cards and then handling whether or not the cards
   * were a match.
   * 
   ...
   */
  public static void playRound(Scanner input, Grid matchingGame) {
    int cardNum1 = askForCardToShow(input, matchingGame);
    int cardNum2 = askForCardToShow(input, matchingGame);
...
//    System.out.println(matchingGame.getGridDisplay());
  ...
  /**
   * This will ask the user for the number of the card they would like to show. If
   * they enter an invalid choice, it will prompt them again until a valid choice
   * is entered.
   * 
   * @param input The Scanner to get input from the console
   ...
   */
  public static int askForCardToShow(Scanner input, Grid matchingGame) {
  ...
  /**
   * This method will check to see if the two selected cards are a match and print
   * out the relevant message. - If they are a match it will also remove those
   * cards from the Grid. - If they are not a match it will hide the cards again.
   * 
   * @param matchingGame The Grid that represents the current game
